...
class MainHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        maxpage = count / 6 + 1
        archives = Archive.select().where((Archive.status == 1) & (Archive.type == "archive")).order_by(Archive.created_time.desc()).offset(0).limit(6)
...
class PageHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        count = Archive.select().count()
        maxpage = count / 6 + 1
        archives = Archive.select().where((Archive.status == 1) & (Archive.type == "archive")).order_by(Archive.created_time.desc()).offset(offset).limit(6)
...
class TagHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        tag = args[0]
        archives = Archive.select().where((Archive.tags.contains(tag)) & (Archive.status == 1) & (Archive.type == "archive")).order_by(Archive.published_time.desc())
        ...
        self.render("timeline.html", archives_groups=archives_groups, first_title=tag)
